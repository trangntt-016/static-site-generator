...
public class JssgApplication implements Runnable {
  ...
  @CommandLine.Option(
      names = {"-i", "--input"},
      description = "Generate HTML website from file/folder")
  private String input;

  // The path to the output directory of the generated files
  @CommandLine.Option(
      names = {"-o", "--output"},
      description = "Specified different output path")
  ...
  @CommandLine.Option(
      names = {"-l", "--lang"},
      description =
          "Specified the language to use when generating the lang attribute on the root <html>"
              + " element")
  ...
  @CommandLine.Option(
      names = {"-c", "--config"},
      description = "Specified the path to the config file")
